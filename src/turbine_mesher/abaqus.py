def write_shell_modal_input(fileName, blade, bladeMesh, adhesiveMat, numModes):
    ...
    for sec in bladeMesh["sections"]:
        snm = sec["elementSet"]
        ln = (
            "*Shell General Section, elset=set_"
            + snm
            + ", composite, orientation=ori_"
            + snm
            + ", offset=0.5, layup=lyp_"
            + snm
            + "\n"
        )
        outFile.write(ln)
